...
async function appendCodexWakeDiagnostic(namespaceDir: string, event: CoordinatorEvent, error: unknown): Promise<void> {
...
): Promise<{ workUnit: string; event: CodexWakeEventV1 | null } | null> {
	if (!event.session_id || !isCodexWakeEventKind(event.kind)) return null;
	if (!(await readCodexHandoff(namespaceDir, event.session_id))) return null;
...
async function publishPendingCodexWakes(
	namespaceDir: string,
	workUnit: string,
	triggeringEvent: CodexWakeEventV1 | null,
): Promise<void> {
	const handoff = await readCodexHandoff(namespaceDir, workUnit);
	if (!handoff) return;
	const attempted = new Set<string>();
	if (triggeringEvent) {
		attempted.add(triggeringEvent.key);
		const outcome = await publishRecordedCodexWake(namespaceDir, handoff, triggeringEvent);
		// A busy thread or a failing transport stops the replay; pending/failed
		// wakes retry on the next wake activity instead of storming the endpoint.
		if (outcome === "thread_active_pending" || outcome === "failed") return;
	}
	for (const event of await listPendingCodexWakeEvents(namespaceDir, workUnit)) {
		if (attempted.has(event.key)) continue;
...
function enqueueCodexWakePublish(
	...
	const previous = codexWakePublishTails.get(namespaceDir) ?? Promise.resolve();
	const next = previous
		.catch(() => undefined)
		.then(() => publishPendingCodexWakes(namespaceDir, workUnit, triggeringEvent))
		.catch(() => undefined);
	codexWakePublishTails.set(namespaceDir, next);
	const cleanup = () => {
		if (codexWakePublishTails.get(namespaceDir) === next) codexWakePublishTails.delete(namespaceDir);
	};
...
export async function awaitCodexWakePublishesForTest(namespaceDir: string): Promise<void> {
	await codexWakePublishTails.get(namespaceDir);
}
...
async function appendCoordinatorEvent(namespaceDir: string, input: CoordinatorEventInput): Promise<CoordinatorEvent> {
	...
	try {
		...
		if (codexWake) enqueueCodexWakePublish(namespaceDir, codexWake.workUnit, codexWake.event);
		return event;
	} finally {
		release();
