...
- Standard (non-ACP) `gjc` sessions can opt into connecting configured runtime MCP servers via the new `mcp.enableRuntimeServers` setting (default **false**). When enabled, MCP servers configured in `~/.gjc/agent/mcp.json` (and, only when `mcp.enableProjectConfig=true`, project-level `.mcp.json` / `.gjc/mcp.json`) are connected and their tools surfaced as active session tools — e.g. CodeGraph via `codegraph serve --mcp`. Default-off preserves the existing MCP quarantine: no manager is constructed, no on-disk MCP config is read or connected, and no `mcp__*` tools register unless the setting is enabled. Project-level config may be read by capability loading but project servers are filtered out before connect unless `mcp.enableProjectConfig=true`. This is distinct from `mcp.discoveryMode` (which only hides already-connected MCP tools behind tool search). ACP sessions (client-supplied `session/new.mcpServers` only) and subagents (MCP-free) are unchanged. Failed MCP servers degrade non-fatally with redacted startup warnings keyed by `mcp:<name>`.

### Fixed

- Hardened runtime MCP startup-warning redaction (`redactMCPStartupError`) against credential leaks and a denial-of-service on the untrusted server/transport failure boundary: exact secret values from server config + process env (now including stdio `args`) are removed structurally with linear `split`/`join`; the input is hard-bounded **before** any regex runs so an unbounded remote HTTP response body can never drive super-linear regex cost (the prior 500-char cap was applied *after* the regex); the credential-shape denylist quantifiers are bounded (the `[a-z0-9_]*secret[a-z0-9_]*` alternative that caused O(n²) backtracking is now `{0,32}`-bounded); and URL userinfo stripping covers username-only (`token@`) forms. Adds a perf regression test asserting a 256 KB adversarial body redacts well under budget.
